...
interface DropboxSuccessResponse {
  link: string;
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = (await response.json()) as DropboxErrorResponse;
      ...
      return Response.json(
        { error: `Failed to get file link: ${errorMessage}` },
        { status: 500 },
      );
    }

    const data = (await response.json()) as DropboxSuccessResponse;

    if (!data.link) {
      return Response.json(
        { error: "No link returned from Dropbox" },
        { status: 500 },
      );
    }
