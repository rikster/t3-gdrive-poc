...
export async function GET(request: NextRequest) {
  ...
  if (!storedTokens) {
    return Response.json({ error: 'Not authenticated with Google Drive' }, { status: 401 });
  }
  ...
  try {
    ...
    if (!file.data || !file.data.webViewLink) {
      return Response.json({ error: 'File not found or no view link available' }, { status: 404 });
    }
